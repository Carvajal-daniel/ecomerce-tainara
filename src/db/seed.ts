...
import { db } from ".";
import { categoryTable, productTable, productVariationTable, bannerTable } from "./schema";

// Definir interfaces para tipagem do productImages
interface ProductImage {
  [color: string]: string[];
}

interface ProductImages {
  [product: string]: ProductImage;
}
...
// Imagens de exemplo para banners
const bannerImages = {
  Acessórios: {
    desktop: "https://onlauri.vtexassets.com/arquivos/ids/1505358/108292_030_1.jpg?v=638858698049900000",
    mobile: "https://onlauri.vtexassets.com/arquivos/ids/1505358/108292_030_1.jpg?v=638858698049900000",
  },
  "Bermuda & Shorts": {
    desktop: "https://d4lgxe9bm8juw.cloudfront.net/banners/bermuda_shorts_desktop.jpg",
    mobile: "https://d4lgxe9bm8juw.cloudfront.net/banners/bermuda_shorts_mobile.jpg",
  },
  Calças: {
    desktop: "https://cea.vtexassets.com/assets/vtex.file-manager-graphql/images/f9258ebc-2caa-4989-a7a4-952404304230___2ea55607d8cf398942ae3ca4b1010154.jpg",
    mobile: "https://cea.vtexassets.com/assets/vtex.file-manager-graphql/images/f9258ebc-2caa-4989-a7a4-952404304230___2ea55607d8cf398942ae3ca4b1010154.jpg",
  },
  Camisetas: {
    desktop: "https://cea.vtexassets.com/assets/vtex.file-manager-graphql/images/fc164b3d-5e07-4e70-a13f-74e7e67135fd___85765eddab8df5b14efb59fdb8958741.jpg",
    mobile: "https://cea.vtexassets.com/assets/vtex.file-manager-graphql/images/fc164b3d-5e07-4e70-a13f-74e7e67135fd___85765eddab8df5b14efb59fdb8958741.jpg",
  },
  "Jaquetas & Moletons": {
    desktop: "https://d4lgxe9bm8juw.cloudfront.net/banners/jaquetas_moletons_desktop.jpg",
    mobile: "https://d4lgxe9bm8juw.cloudfront.net/banners/jaquetas_moletons_mobile.jpg",
  },
  Tênis: {
    desktop: "https://d4lgxe9bm8juw.cloudfront.net/banners/tenis_desktop.jpg",
    mobile: "https://d4lgxe9bm8juw.cloudfront.net/banners/tenis_mobile.jpg",
  },
};
...
const categories = [
  {
    name: "Acessórios",
    description: "Acessórios para todas as ocasiões",
  },
  {
    name: "Bermuda & Shorts",
...
const banners = [
  {
    name: "Banner Acessórios",
    categoryName: "Acessórios",
  },
  {
    name: "Banner Bermuda & Shorts",
  ...
  {
    name: "Banner Camisetas",
    categoryName: "Camisetas",
  },
  {
    name: "Banner Jaquetas & Moletons",
...
async function main() {
  ...
  try {
    ...
    await db.delete(productVariationTable).catch((err) => {
      console.warn(`⚠️ Tabela product_variation não encontrada, pulando exclusão: ${err.message}`);
    });
    await db.delete(productTable).catch((err) => {
      console.warn(`⚠️ Tabela product não encontrada, pulando exclusão: ${err.message}`);
    });
    await db.delete(bannerTable).catch((err) => {
      console.warn(`⚠️ Tabela banner não encontrada, pulando exclusão: ${err.message}`);
    });
    await db.delete(categoryTable).catch((err) => {
      console.warn(`⚠️ Tabela category não encontrada, pulando exclusão: ${err.message}`);
    });
    ...
    for (const bannerData of banners) {
      ...
      const categoryId = categoryMap.get(bannerData.categoryName);

      if (!categoryId) {
        throw new Error(
          `Categoria "${bannerData.categoryName}" não encontrada para o banner ${bannerData.name}`,
        );
      }

      const bannerImagesForCategory = bannerImages[bannerData.categoryName as keyof typeof bannerImages];

      console.log(`  🖼️ Criando banner: ${bannerData.name}`);

      await db.insert(bannerTable).values({
        id: bannerId,
        name: bannerData.name,
        desktop_image: bannerImagesForCategory?.desktop || "https://via.placeholder.com/1920x600",
        mobile_image: bannerImagesForCategory?.mobile || "https://via.placeholder.com/600x800",
        category_id: categoryId,
        slug: generateSlug(bannerData.name),
      });
    }
    ...
    for (const productData of products) {
      ...
      // Obter a primeira imagem disponível para o produto
      const productKey = productData.name as keyof ProductImages;
      const productImageObj = productImages[productKey];
      const firstColor = productImageObj ? Object.keys(productImageObj)[0] : undefined;
      const productImage = firstColor && productImageObj && productImageObj[firstColor] ? productImageObj[firstColor][0] : "https://via.placeholder.com/150";
      ...
      for (const variantData of productData.variants) {
        ...
        const productKey = productData.name as keyof ProductImages;
        const variantImages = productImages[productKey]?.[variantData.color] || [];
        ...
        await db.insert(productVariationTable).values({
          ...
          image_url: variantImages[0] || "https://via.placeholder.com/150",
    ...
    console.log(
      `📊 Foram criadas ${categories.length} categorias, ${banners.length} banners, ${
        products.length
